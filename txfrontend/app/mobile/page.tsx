...
export default function MobilePage() {
  ...
  const handleGetTicket = (serviceId: number) => {
    try {
      const newTicket = createTicket(serviceId, mobilePhone || undefined)
      const allTickets = getTicketsWithRelations()
      const ticketWithRelations = allTickets.find(t => t.id === newTicket.id)
      
      setUserTicket(ticketWithRelations)
      setHasTicket(true)
      localStorage.setItem('mobile-ticket-id', newTicket.id.toString())
    } catch (error) {
        ...
        <div className="max-w-md mx-auto">
          ...
          {userTicket.status === Status.COMPLETED ? (
            ...
            <Card className="mb-6 bg-gradient-to-r from-blue-500 to-purple-600 text-white">
              <CardContent className="p-6 text-center">
                <div className="text-5xl font-bold mb-2">{userTicket.number}</div>
                <p className="text-blue-100 mb-4">{userTicket.service.name}</p>
          ...
          <Card className="mb-6">
            ...
            <CardContent>
              <div className="space-y-2 text-sm">
                ...
                <p>
                  <strong>Servicio:</strong> {userTicket.service.name}
                </p>
              </div>
            </CardContent>
          </Card>
